...
template <typename T> using xtensor_t = xt::xexpression<T>;
...
template <typename T> size_t max_index(const T &vec) {
...
template <typename Container> size_t max_size(const Container &c) {
...
template <typename T> struct index3d {
...
template <class ValType> using value_is_float = value_type_is<ValType, float>;

template <class ValType> using value_is_double = value_type_is<ValType, double>;

// check that whole tuple are xexpressions
template <typename> struct is_xexpression : std::false_type {};
...
struct are_all<checker, T0, Ts...>
    : std::integral_constant<bool, checker<T0>::value &&
                                       are_all<checker, Ts...>::value> {};

template <typename... Ts> using are_all_xexpr = are_all<is_xexpression, Ts...>;
} // namespace tiny_dnn
